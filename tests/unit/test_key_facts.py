...
def test_note_parser_parses_key_fact_tag():
    ...
    assert fact["statement"] == "App sẽ hỗ trợ 10k DAU"
...
def test_note_parser_key_fact_optional_fields_default_to_empty():
    ...
    assert len(result["key_facts"]) == 1
    fact = result["key_facts"][0]
    assert fact["statement"] == "DAU target là 10k"
    assert fact["source"] == ""
    assert fact["turn"] == ""
...
def test_note_parser_key_fact_and_assumption_in_same_content():
    content = (
        "KEY_FACT: DAU là 10k | source: user | turn: 1\n"
        "ASSUMPTION: Người dùng mobile | source: pm | confidence: high | impact: high | owner: pm | status: open"
    )
    result = extract_structured_objects(content)
    assert len(result["key_facts"]) == 1
...
def test_note_parser_unrecognized_lines_ignored():
    content = "Đây là note thông thường không có tag\nKEY_FACT: Fact thật | source: dev | turn: 3"
    result = extract_structured_objects(content)
    assert len(result["key_facts"]) == 1
...
def test_key_facts_block_contains_statement_when_facts_present():
    state = _state()
    state["key_facts"] = [{"statement": "DAU là 10k", "source": "user", "turn": "1"}]
    block = _build_key_facts_block(state)
    assert "DAU là 10k" in block
...
def test_build_tool_selection_prompt_contains_key_facts_when_non_empty():
    ...
    assert "Budget tối đa 500tr" in prompt
...
async def test_summarize_node_does_not_clear_key_facts():
    ...
    from app.graphs.nodes import summarize_node, route_before_analyze
    ...
    with patch("app.graphs.nodes.route_before_analyze", return_value="summarize"):
        mock_llm = AsyncMock()
        mock_llm.generate = AsyncMock(return_value=({"summary": "tóm tắt"}, {}))

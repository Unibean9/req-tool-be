...
class ArtifactPriority(str, enum.Enum):
    MUST = "must"
    SHOULD = "should"
    COULD = "could"
    WONT = "wont"
...
class WorkflowStepPhase(str, enum.Enum):
    BRD = "brd"
    SRS = "srs"
    DELIVERY = "delivery"
...
class SourceDocument(AuditMixin, Base):
    ...
    uploaded_by_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, index=True)
...
class WorkflowRun(AuditMixin, Base):
    ...
    extra_metadata: Mapped[Any] = jsonb_column("metadata", nullable=False, default=dict)
    created_by_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, index=True)
...
class Artifact(AuditMixin, Base):
    ...
    project_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("projects.id"), nullable=False, index=True)
    run_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("workflow_runs.id"), nullable=True, index=True)
    step_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("workflow_steps.id"), nullable=True, index=True)
    ...
    status: Mapped[ArtifactStatus] = enum_column(ArtifactStatus, nullable=False, default=ArtifactStatus.DRAFT, index=True)
...
class ArtifactVersion(AuditMixin, Base):
    ...
    artifact_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("artifacts.id"), nullable=False, index=True)
    ...
    parent_version_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("artifact_versions.id"), nullable=True, index=True)
    created_by_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, index=True)
    source_document_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("source_documents.id"), nullable=True, index=True)
    agent_run_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_runs.id"), nullable=True, index=True)
    ...
    provider_config_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("llm_provider_configs.id"), nullable=True, index=True)
...
class ArtifactLink(AuditMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("source_artifact_id", "target_artifact_id", "relation_type", name="uq_artifact_links_pair_type"),
    )

    project_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("projects.id"), nullable=False, index=True)
    source_artifact_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("artifacts.id"), nullable=False, index=True)
    target_artifact_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("artifacts.id"), nullable=False, index=True)
...
class ArtifactEvidence(AuditMixin, Base):
    ...
    artifact_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("artifacts.id"), nullable=False, index=True)
    artifact_version_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("artifact_versions.id"), nullable=True, index=True)
    source_document_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("source_documents.id"), nullable=True, index=True)
...
class ArtifactReview(AuditMixin, Base):
    ...
    artifact_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("artifacts.id"), nullable=False, index=True)
    artifact_version_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("artifact_versions.id"), nullable=True, index=True)
    reviewed_by_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, index=True)
...
class WorkflowStep(AuditMixin, Base):
    ...
    status: Mapped[WorkflowStepStatus] = enum_column(WorkflowStepStatus, nullable=False, default=WorkflowStepStatus.PENDING)
    ...
    approved_by_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, index=True)
    extra_metadata: Mapped[Any] = jsonb_column("metadata", nullable=False, default=dict)

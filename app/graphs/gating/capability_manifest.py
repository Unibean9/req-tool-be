"""Declarative manifest for capabilities this phase migrates to `CapabilityResolver`.
...
evaluation logic here — see the risk register in phase-03-capability-resolver-shadow-enforcement.md
("manifest becomes an executable god-object").
"""
...
# Read-only pilot: no artifact mutation, no HITL interrupt. Matches the phase brief's suggested
# candidate set exactly (run_critique / run_readiness_check / recommend_next_workflow) — verified
# against app/graphs/tool_metadata.py (none interrupt, none mutate the artifact/document graph; the
# two that write are best-effort AgentToolCall audit inserts that never gate the response).
...
# until their fenced command phases (Phase 4/5) land. create_artifact_link/propose_retirement are

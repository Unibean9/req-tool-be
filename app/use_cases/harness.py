...
USE_CASE_GROUP_DETAIL_SYSTEM_PROMPT = (
    ...
    "Choose actors from the supplied roster only, include complete detail fields when the source supports "
    "them, and cite exact evidence IDs. The `sourceUseCases` list is the deterministic floor: enrich "
    "those rows only, preserve each row's ID in `local_tag`, and do not add or remove rows. "
